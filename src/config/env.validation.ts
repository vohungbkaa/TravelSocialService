...
export const envSchema = z
  ...
  .superRefine((env, context) => {
    const hasFacebookAppId = Boolean(env.FACEBOOK_APP_ID?.trim());
    const hasFacebookAppSecret = Boolean(env.FACEBOOK_APP_SECRET?.trim());
    if (hasFacebookAppId === hasFacebookAppSecret) return;

    const missingKey = hasFacebookAppId
      ? 'FACEBOOK_APP_SECRET'
      : 'FACEBOOK_APP_ID';
    context.addIssue({
      code: 'custom',
